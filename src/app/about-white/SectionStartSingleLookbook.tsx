function SectionStartSingleLookbook() {
  return (
    <>
      <div className="tmp-lookbook-portion portion-md" data-tmp-position-vertical="23" data-tmp-position-horigental="15">
      <div className="tmp-lookbook-thumb">
      <img src="/assets/images/avatar/avatar8.webp" alt="Thumbnail Image" />
      </div>
      <div className="tmp-lookbook-content-box-wrapper">
      <button className="tmp-lookbook-close-btn"><i className="fas fa-times"></i></button>
      <div className="tmp-lookbook-content-box placed-right">
      <div className="tmp-message-shape"></div>
      <div className="tmp-inner">
      <p className="b2 tmp-text-heading mb--20">
                                                  “Their team is always available to answer my questions. I trust them
                                                  completely with my business insurance.”
                                              </p>
      <div className="tmp-separator tmp-bg-primary-opacity-400 mb--10"></div>
      <div className="b4 d-flex justify-content-between">
      <span>
      <span className="tmp-fw-medium tmp-text-heading">Lisa W.</span> - UI/UX
                                                  Designer</span>
      <span className="tmp-fw-medium">
      <span>
      <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 10 15" fill="none">
      <path d="M5.52447 1.15102C5.67415 0.690369 6.32585 0.69037 6.47553 1.15103L7.45934 4.17889C7.52628 4.3849 7.71826 4.52438 7.93487 4.52438H11.1186C11.6029 4.52438 11.8043 5.14419 11.4124 5.42889L8.83679 7.30021C8.66155 7.42753 8.58822 7.65322 8.65516 7.85923L9.63897 10.8871C9.78864 11.3477 9.2614 11.7308 8.86955 11.4461L6.29389 9.57479C6.11865 9.44747 5.88135 9.44747 5.70611 9.57479L3.13045 11.4461C2.73859 11.7308 2.21136 11.3477 2.36103 10.8871L3.34484 7.85923C3.41178 7.65322 3.33845 7.42753 3.16321 7.30021L0.587553 5.42889C0.195696 5.14419 0.397084 4.52438 0.881446 4.52438H4.06513C4.28174 4.52438 4.47372 4.3849 4.54066 4.17889L5.52447 1.15102Z" fill="#F7A51E"></path>
      </svg>
      </span>
                                                  4.9
                                                  </span>
      </div>
      </div>
      </div>
      </div>
      </div>

    </>
  )
}

export default SectionStartSingleLookbook
